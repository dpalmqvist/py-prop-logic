...
from prop_logic.expression import make_expr
...
class TestPropLogic(unittest.TestCase):
    def test_cats(self):
        ...
        for (question, answer) in zip(to_ask, answers):
            ...
            results = [r for r in my_kb.ask(question)]
    ...
    def test_weapon_sales(self):
        ...
        for (question, answer) in zip(to_ask, answers):
            print "Asking %s" % question
            result = [r for r in my_kb.ask(make_expr(question))]
            self.assertEquals(answer, result)

        print "Retracting NewYorker(East)"
        my_kb.retract("NewYorker(East)")
        print "Asking Criminal(x)"
        result = [r for r in my_kb.ask("Criminal(x)")]
    ...
    def test_wumpus(self):
        ...
        for (question, answer) in zip(to_ask, answers):
            print "Asking  %s?" % question
            result = [r for r in my_kb.ask(question)]
